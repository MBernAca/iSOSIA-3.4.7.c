...
meshtype read_mesh(char *path,char file[200],FILE *fp)
{
  ...
  fscanf(fp,"%lf\n",&mesh.maxtime); 
  fscanf(fp,"%d\n",&mesh.nFiletime);
  mesh.filetime[0] = (double*) malloc(mesh.nFiletime*sizeof(double));
  for (j=0;j<mesh.nFiletime;j++) {
	fscanf(fp,"%lf\n",&mesh.filetime[0][j]);
  }
  // fscanf(fp,"%lf\n",&mesh.filetime); 
  fscanf(fp,"%lf\n",&mesh.maxdt); 
  ...
  fscanf(fp,"%d\n",&mesh.dosediment); 
  fscanf(fp,"%d\n",&mesh.doglacialsedi); 
  fscanf(fp,"%d\n",&mesh.dosubglacialdeformation); 
  fscanf(fp,"%d\n",&mesh.doparticles); 
  fscanf(fp,"%d\n",&mesh.dodeposit); 
  fscanf(fp,"%d\n",&mesh.dodebrisablation); 
  fscanf(fp,"%d\n",&mesh.douniformerosion); 
  fscanf(fp,"%d\n",&mesh.dopulse); 
  fscanf(fp,"%d\n",&mesh.Flaw); //Maxime
  fscanf(fp,"%d\n",&mesh.gelaw); //Maxime
  fscanf(fp,"%d\n",&mesh.steadybed); //Maxime
  fscanf(fp,"%d\n",&mesh.nbCores); //Maxime
  fclose(fp);
...
mproptype read_mprop(char *path,char file[200],FILE *fp)
{
  ...
  fread(&mprop.scc,sizeof(double),1,fp); //Maxime
  ...
  mprop.Mrate_T = malloc(4*sizeof(double*)); for (i=0;i<4;i++) mprop.Mrate_T[i] = malloc(mprop.nMrate_T*sizeof(double)); //Maxime
  for (j=0;j<mprop.nMrate_T;j++) for (i=0;i<4;i++) fread(&mprop.Mrate_T[i][j],sizeof(double),1,fp);
  fclose(fp);
...
hwproptype read_hwprop(char *path,char file[200],FILE *fp)
{
  ...
  fread(&hwprop.dss,sizeof(double),1,fp);
  fclose(fp);
...
hproptype read_hprop(char *path,char file[200],FILE *fp)
{
  ...
  fread(&hprop.kt,sizeof(double),1,fp); //Maxime
  fclose(fp);
...
fproptype read_fprop(char *path,char file[200],FILE *fp)
{
  ...
  fread(&fprop.m,sizeof(double),1,fp); //Maxime
  fread(&fprop.t_sp,sizeof(int),1,fp); //Maxime
  fclose(fp);
...
void write_output(celltype **cells,hptype **hp,vptype **vp,meshtype mesh,char *path,char file[200],FILE *fp,long step)
{
  ...
  if ((fp = fopen(file,"wb")) == NULL) {
    ...
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].avasite,sizeof(int),1,fp);
    ...
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].snow,sizeof(double),1,fp); 
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].np,sizeof(int),1,fp);
	for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].npia,sizeof(int),1,fp);//Maxime
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].sedi,sizeof(double),1,fp);
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].water2surf,sizeof(double),1,fp);
    ...
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].bwater,sizeof(double),1,fp);
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].subglacial_erosion,sizeof(double),1,fp);
    ...
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].Tair,sizeof(double),1,fp);
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].Tb,sizeof(double),1,fp);
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].lakewater,sizeof(double),1,fp);
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].margin,sizeof(int),1,fp);
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].curv,sizeof(double),1,fp);
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].sdeformation,sizeof(double),1,fp);
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].Qw,sizeof(double),1,fp);
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].bQw,sizeof(double),1,fp);
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].water,sizeof(double),1,fp);
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].Vs[0],sizeof(double),1,fp);/*Maxime*/
    ...
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].streamload,sizeof(double),1,fp);
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].cbs,sizeof(double),1,fp);
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].Qt,sizeof(double),1,fp);/*Maxime*/
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].smelt,sizeof(double),1,fp); /*Maxime*/
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].tslope,sizeof(double),1,fp);/*Maxime*/
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].hslope,sizeof(double),1,fp);/*Maxime*/
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].Vs[19],sizeof(double),1,fp);/*Maxime*/
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].theta,sizeof(double),1,fp); /*Maxime*/
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].hillslope_erate,sizeof(double),1,fp); /*Maxime*/
	for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].landslide_erosion,sizeof(double),1,fp); /*Maxime*/
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].reciever,sizeof(int),1,fp);
    for (j=0;j<nx;j++) for (i=0;i<ny;i++) fwrite(&cells[i][j].vy_d,sizeof(double),1,fp);
    for (j=0;j<nx+1;j++) for (i=0;i<ny;i++) fwrite(&hp[i][j].dbdx,sizeof(double),1,fp);
    for (j=0;j<nx;j++) for (i=0;i<ny+1;i++) fwrite(&vp[i][j].dbdy,sizeof(double),1,fp);
...
void write_particles(particletype *pp,long npa,long *pactive,char *path,char file[200],FILE *fp,long step)
{
  ...
  if ((fp = fopen(file,"wb")) == NULL) {
	...
	// for (i=0;i<npa;i++) fwrite(&pp[pactive[i]].vx_d,sizeof(double),1,fp); /*Maxime*/
	// for (i=0;i<npa;i++) fwrite(&pp[pactive[i]].vx_b,sizeof(double),1,fp);
    for (i=0;i<npa;i++) fwrite(&pp[pactive[i]].vy,sizeof(double),1,fp);
	// for (i=0;i<npa;i++) fwrite(&pp[pactive[i]].vy_d,sizeof(double),1,fp); /*Maxime*/
	// for (i=0;i<npa;i++) fwrite(&pp[pactive[i]].vy_b,sizeof(double),1,fp);
    for (i=0;i<npa;i++) fwrite(&pp[pactive[i]].vz,sizeof(double),1,fp);
    for (i=0;i<npa;i++) fwrite(&pp[pactive[i]].erate,sizeof(double),1,fp);
	for (i=0;i<npa;i++) fwrite(&pp[pactive[i]].velocity,sizeof(double),1,fp); /*Maxime*/
	for (i=0;i<npa;i++) fwrite(&pp[pactive[i]].type,sizeof(int),1,fp); /*Maxime*/
    fclose(fp);
  }
