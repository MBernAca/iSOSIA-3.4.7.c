void sia(celltype **cells,hptype **hp,vptype **vp,cornertype **cp,meshtype *mesh,iproptype iprop)
{

  int i,j;
  double tx,ty,ts,vb,beta,vx,vy;
...
#pragma omp parallel shared(cells,vp,hp,cp,mesh,maxspeed,maxdt) private(i,j,tx,ty,ts,vb,beta,vx,vy,dtfac) firstprivate(nx,ny,dx,dy,lmaxspeed,gamma,gamma0,Cs,C,L0,minbeta,maxsliding,dosliding,smode,maxdef,minice,lmaxdt)
  {


    /*average velocity in h-points*/
#pragma omp for schedule(static) 
    for (i=0;i<ny;i++) {
      for (j=1;j<nx;j++) {
	if (hp[i][j].ice > minice) {
	  hp[i][j].vx_d = -2.0/5.0*gamma*hp[i][j].alpha2*hp[i][j].dhdx*pow(hp[i][j].ice,4.0); 
	  if (hp[i][j].vx_d < -maxdef) hp[i][j].vx_d = -maxdef;
	  if (hp[i][j].vx_d > maxdef) hp[i][j].vx_d = maxdef;
	}
	else {
	  hp[i][j].vx_d = 0.0;
    ...
    /*average velocity in v-points*/
#pragma omp for schedule(static) 
    for (i=1;i<ny;i++) {
      for (j=0;j<nx;j++) {
	if (vp[i][j].ice > minice) {
	  vp[i][j].vy_d = -2.0/5.0*gamma*vp[i][j].alpha2*vp[i][j].dhdy*pow(vp[i][j].ice,4.0);
	  if (vp[i][j].vy_d < -maxdef) vp[i][j].vy_d = -maxdef;
	  if (vp[i][j].vy_d > maxdef) vp[i][j].vy_d = maxdef;
	}
	else {
	  vp[i][j].vy_d = 0.0;
...
#pragma omp for schedule(static) 
	...
	for (j=0;j<nx;j++) {
	  ...
	  cells[i][j].pb = (1.0+pow(cells[i][j].dbdx,2.0)+pow(cells[i][j].dbdy,2.0))*cells[i][j].ice;
	  cells[i][j].sxz = -cells[i][j].dhdx*cells[i][j].ice;
	  cells[i][j].syz = -cells[i][j].dhdy*cells[i][j].ice;
	  cells[i][j].tn = cells[i][j].pb+(2.0*cells[i][j].dbdx*cells[i][j].sxz+2.0*cells[i][j].dbdy*cells[i][j].syz)/pow(cells[i][j].lb,2.0);
	  ...
	  cells[i][j].tbx = (cells[i][j].dbdx*cells[i][j].pb+cells[i][j].sxz-cells[i][j].dbdx*cells[i][j].tn)/cells[i][j].lb;
	  cells[i][j].tby = (cells[i][j].dbdy*cells[i][j].pb+cells[i][j].syz-cells[i][j].dbdy*cells[i][j].tn)/cells[i][j].lb;
	  cells[i][j].tbz = (-cells[i][j].dbdx*cells[i][j].sxz-cells[i][j].dbdy*cells[i][j].syz-cells[i][j].pb+cells[i][j].tn)/cells[i][j].lb;
...
#pragma omp for schedule(static) nowait
	...
	for (j=1;j<nx;j++) {
	  ...
	  if (hp[i][j].ice > 1.0) hp[i][j].vx_b = vb*tx/(ts+1.0e-6);
...
#pragma omp for schedule(static)
	...
	for (j=0;j<nx;j++) {
	  ...
	  if (vp[i][j].ice > 1.0) vp[i][j].vy_b = vb*ty/(ts+1.0e-6);
...
#pragma omp for schedule(static)
	...
	/*dtfac = (*mesh).ct*dx/fabs(hp[i][j].vx);*/
...
#pragma omp for schedule(static)
	...
	/*dtfac = (*mesh).ct*dy/fabs(vp[i][j].vy);*/
	if (lmaxdt > dtfac) lmaxdt = dtfac;
      }
    }
#pragma omp critical
...
void isosia(celltype **cells,hptype **hp,vptype **vp,cornertype **cp,meshtype *mesh,iproptype iprop)
{
  ...
  double Cs = iprop.Cs;
  double minbeta = iprop.minbeta;
  double maxdef = iprop.maxdeformation; //MAxime
  double maxsliding = iprop.maxsliding; //Maxime
...
#pragma omp parallel shared(cells,vp,hp,cp,mesh,stressit,sres,sres_old,sdiff,stot,veloitt,vres,vres_old,vdiff,vtot,maxspeed,maxRes,maxdt,ndiv,divcount,ifac) private(i,j,tx,ty,ts,vb,beta,vx,vy,Res,lmaxRes,dtfac,fac) firstprivate(nx,ny,dx,dy,periodic,lmaxspeed,gamma,gamma0,Cs,sfac,vbfac,maxitt_v,maxitt_s,C,L0,minbeta,dosliding,smode,gmode,minice,lmaxdt,dum,fc,maxdef,maxsliding)
  {
...
#pragma omp for schedule(static)
	...
	for (j=0;j<nx;j++) {
	  if ((cells[i][j].ice > minice)&&(gmode > 0)) {
	    cells[i][j].exx = (hp[i][j+1].vx-hp[i][j].vx)/dx-(cells[i][j].dhdx*((hp[i][j].vx_s-hp[i][j].vx)+(hp[i][j+1].vx_s-hp[i][j+1].vx))/2.0+cells[i][j].dtdx*((hp[i][j].vx-hp[i][j].vx_b)+(hp[i][j+1].vx-hp[i][j+1].vx_b))/2.0)/(cells[i][j].ice+100.0);
	    cells[i][j].eyy = (vp[i+1][j].vy-vp[i][j].vy)/dy-(cells[i][j].dhdy*((vp[i][j].vy_s-vp[i][j].vy)+(vp[i+1][j].vy_s-vp[i+1][j].vy))/2.0+cells[i][j].dtdy*((vp[i][j].vy-vp[i][j].vy_b)+(vp[i+1][j].vy-vp[i+1][j].vy_b))/2.0)/(cells[i][j].ice+100.0);
	    cells[i][j].exy = 0.25*(cp[i][j].exy+cp[i+1][j].exy+cp[i+1][j+1].exy+cp[i][j+1].exy)-(cells[i][j].dhdy*((hp[i][j].vx_s-hp[i][j].vx)+(hp[i][j+1].vx_s-hp[i][j+1].vx))/2.0+cells[i][j].dhdx*((vp[i][j].vy_s-vp[i][j].vy)+(vp[i+1][j].vy_s-vp[i+1][j].vy))/2.0+cells[i][j].dtdy*((hp[i][j].vx-hp[i][j].vx_b)+(hp[i][j+1].vx-hp[i][j+1].vx_b))/2.0+cells[i][j].dtdx*((vp[i][j].vy-vp[i][j].vy_b)+(vp[i+1][j].vy-vp[i+1][j].vy_b))/2.0)/(2.0*cells[i][j].ice+100.0);
...
#pragma omp for schedule(static) reduction(+:vdiff) reduction(+:vtot)
	...
	for (j=1;j<nx;j++) {
	  if (hp[i][j].ice > minice) {
		...
		// if (hp[i][j].vx_d > maxdef) hp[i][j].vx_d = maxdef;
		// else if (hp[i][j].vx_d < - maxdef) hp[i][j].vx_d = -maxdef;
	...
	hp[i][0].vx_d = 0.0;//hp[i][1].vx_d;
    hp[i][nx].vx_d = 0.0;//hp[i][nx-1].vx_d;
...
#pragma omp for schedule(static) reduction(+:vdiff) reduction(+:vtot)
	...
	for (j=1;j<nx-1;j++) {
	  ...
	  if (vp[i][j].ice > minice) {
		...
		// if (vp[i][j].vy_d > maxdef) vp[i][j].vy_d = maxdef;
		// else if (vp[i][j].vy_d < - maxdef) vp[i][j].vy_d = -maxdef;
	    if (Res > lmaxRes) lmaxRes = Res;
	 ...
	 for (i=0;i<ny;i++) {
	   for (j=0;j<nx;j++) { 
	    if (cells[i][j].margin < 0.0) {
	      cells[i][j].lb = sqrt(1.0+pow(cells[i][j].dtdx,2.0)+pow(cells[i][j].dtdy,2.0));
	    ...
	    cells[i][j].tn = cells[i][j].pb+(2.0*cells[i][j].dtdx*cells[i][j].sxz+2.0*cells[i][j].dtdy*cells[i][j].syz-pow(cells[i][j].dtdx,2.0)*cells[i][j].sxx-pow(cells[i][j].dtdy,2.0)*cells[i][j].syy-cells[i][j].szz-2.0*cells[i][j].dtdx*cells[i][j].dtdy*cells[i][j].sxy)/pow(cells[i][j].lb,2.0);
	    /*cells[i][j].tn = cells[i][j].ice;*/
	    if (cells[i][j].tn < 0.0) cells[i][j].tn = 0.0;
	    /*cells[i][j].te = cells[i][j].tn-cells[i][j].Pw; if (cells[i][j].te < 0.01*cells[i][j].ice) cells[i][j].te = 0.01*cells[i][j].ice;*/
	    cells[i][j].tbx = (cells[i][j].dtdx*(cells[i][j].pb-cells[i][j].sxx)-cells[i][j].dtdy*cells[i][j].sxy+cells[i][j].sxz-cells[i][j].dtdx*cells[i][j].tn)/cells[i][j].lb;
	    cells[i][j].tby = (-cells[i][j].dtdx*cells[i][j].sxy+cells[i][j].dtdy*(cells[i][j].pb-cells[i][j].syy)+cells[i][j].syz-cells[i][j].dtdy*cells[i][j].tn)/cells[i][j].lb;
	    cells[i][j].tbz = (-cells[i][j].dtdx*cells[i][j].sxz-cells[i][j].dtdy*cells[i][j].syz-cells[i][j].pb+cells[i][j].szz+cells[i][j].tn)/cells[i][j].lb;
	    cells[i][j].ts = sqrt(pow(cells[i][j].tbx,2.0)+pow(cells[i][j].tby,2.0)+pow(cells[i][j].tbz,2.0));
	    if (smode == 0) {
	      if (cells[i][j].ice > 10.0) cells[i][j].vb = Cs*pow(cells[i][j].ts,2.0);
	      else if (cells[i][j].ice > 1.0) cells[i][j].vb = (Cs*cells[i][j].ice/10.0)*pow(cells[i][j].ts,2.0);
	      ...
	      if (beta < minbeta) beta = minbeta;
	      if (cells[i][j].ice > 10.0) cells[i][j].vb = L0*pow(cells[i][j].ts,3.0)/beta;
	      else if (cells[i][j].ice > 1.0) cells[i][j].vb = (L0*cells[i][j].ice/10.0)*pow(cells[i][j].ts,3.0)/beta;
	      else cells[i][j].vb = 0.0;	    
	    }
	    else if (smode == 3) {
	      ...
	      if (cells[i][j].ice > 100.0) cells[i][j].vb = Cs*pow(cells[i][j].ts,3.0)/beta;
	      else if (cells[i][j].ice > 10.0) cells[i][j].vb = (Cs*cells[i][j].ice/100.0)*pow(cells[i][j].ts,3.0)/beta;
	      else cells[i][j].vb = 0.0; 
...
#pragma omp for schedule(static)
      for (i=0;i<ny;i++) {
	for (j=0;j<nx+1;j++) {
	  hp[i][j].vx_s = 1.2*hp[i][j].vx + hp[i][j].vx_b;
	}
      }
#pragma omp for schedule(static)
      for (i=0;i<ny+1;i++) {
	for (j=0;j<nx;j++) {
	  vp[i][j].vy_s = 1.2*vp[i][j].vy + vp[i][j].vy_b; 
	}
      }
...
void depositsnow(celltype **cells, mproptype mprop, int i,int j,double sc,double cc)
{
  // Maxime - mprop
  int k,nne,maxk;
  ...
  for (k=1;k<nne;k++) {
	...
	/*recursive call*/
	depositsnow(cells,mprop,cells[i][j].ne_i[maxk],cells[i][j].ne_j[maxk],scc,cc);
...
void avalance(celltype **cells,meshtype mesh,mproptype mprop,sortarraytype *sortarray,double dt)
{
	/*See Egholm et al. (2015)*/
  ...
  //double accrate = 0.0; //Maxime
  double tacr = 1.0; //Maxime
  ...
  for (indx=0;indx<nn;indx++) {
    ...
    if (cells[i][j].snow > 1e-3) depositsnow(cells,mprop,i,j,sc,cc); //Maxime - mprop
  ...
  for (i=0;i<ny;i++) {
    for (j=0;j<nx;j++) {
		/*Limite rate of accumulation rate*/
		// accrate = pos*cells[i][j].snow/dts;
		// if ((accrate - cells[i][j].accrate)/dt > tacr) {
			// cells[i][j].accrate = tacr*dt+cells[i][j].accrate;
		// }
		// else {
...
void get_mrate_T(celltype **cells,meshtype mesh,mproptype mprop)
{
  ...
  for (i=0;i<ny;i++) {
	...
	smelt = mprop.Mrate_T[2][k]; /*Maxime*/
      }
      else {
	...
	smelt = (1.0-fac)*mprop.Mrate_T[2][k-1] + fac*mprop.Mrate_T[2][k];
	  ...
	  cells[i][j].smelt = cells[i][j].mrate*smelt; //Maxime 
      }
      else {
	cells[i][j].accrate = 0.0;
	cells[i][j].smelt = maxabla;
...
void mass_balance(celltype **cells,meshtype *mesh,iproptype iprop,mproptype mprop,hwproptype hwprop,double time,double dt)
{
  ...
  double ksg = 1e-2; //Maxime
  double rho_i = iprop.rho;
  double rho_w = 1000; //Maxime
...
#pragma omp for schedule(static)
	...
	if ((*mesh).doglacialhydrology > 0) {
		cells[i][j].water2surf = 0.0;
		/* Pass portion of surface melting to surface water */
		cells[i][j].water2surf += ((1-hwprop.a2w)*smelt)/dt;
		/* The remaining part goes to the base of the ice */
		cells[i][j].bwater = (hwprop.a2w*smelt + bmelt + imelt)/dt; /*m yr-1 - bwater maxime ksg*rho_i/rho_w*smelt +*/
	}
...
void glacial_erosion(celltype **cells,hptype **hp,vptype **vp,iproptype iprop,meshtype *mesh,double dt,double time)
{
  ...
  double Cdt = 0.1; /*Maxime*/
  int nci=(*mesh).nci; //Maxime = number of cell included
  int gelaw = (*mesh).gelaw; //Maxime - glacial erosion law
...
#pragma omp for schedule(static)
	...
	if ((cells[i][j].sliding > 0.01)&&(cells[i][j].margin < 0)) {
	  ...
	  pe = 0.01*cells[i][j].te; //te normalized by rho_ice*g, so pe = (rho_ice*g)*te/1e6 (MPa)
	  if (pe < 0.0) pe = 0.0; if (pe > 5.0) pe = 5.0;
	  if (slope > -0.55) {
	    if (slope > 1.0) slope = 1.0;
	    //qtot = pow(pe,3.0)*pow(ub,1.0)*pow(slope+0.55,2.0);
		qtot = pow(ub,2.0);
		}
	    /*qtot = pow(pe,3.0)*pow(ub,1.0);*/
	  else {
		qtot = 0.0;
	  }
	  ...
	  // Abrasion
	  if (gelaw == 0) {
		atot = cells[i][j].Ka*pow(ub,ap)*sqrt(1.0+pow(cells[i][j].bslope,2.0))*cos(cells[i][j].bslope);
	  }
	  else {
		  if (slope > -0.55) {
		  atot = cells[i][j].Ka*pow(ub,ap)*sqrt(1.0+pow(cells[i][j].bslope,2.0))*pow(cells[i][j].bslope+0.55,2.0)*pow(pe,3.0);
		  }
	  }
	  
	  if ((doparticles > 0)) { 
	    /*efac = cells[i][j].afac;*/
		efac = cells[i][j].afac * exp(-cells[i][j].Vbsedi/Cdt); /*Maxime*/
	  ...
	  if (doglacialsedi > 0) { // Add basal sediment effect
	    //efac = cells[i][j].cbs*cells[i][j].angular;
		/* Cover function */
		efac =  exp((-cells[i][j].sedi)/Cdt);
	    if (efac < minefac) efac = minefac;  
	    //atot = Ka*efac*cells[i][j].ts*ub*sqrt(1.0+pow(cells[i][j].bslope,2.0));//*(1.0-cells[i][j].SLf); //Maxime - glacial erosion limited by ice-bed contact
		atot *= efac;//*(1.0-cells[i][j].SLf);//Maxime - glacial erosion limited by ice-bed contact
	  }
	  ...
	  if (doglacialsedi > 0) {
	    ...
	    cells[i][j].sedi += sedifac*(qtot+atot)*dt;
...
#pragma omp single
    {
	  ...
	  meanqrate /= (double)(nci);
      meanarate /= (double)(nci);
...
void glacial_sediment_transport(celltype **cells,hptype **hp,vptype **vp,meshtype mesh,iproptype iprop,double dt)
{
  ...
  /*See Ugelvig et al. (2018) and Rowan et al. (2015)*/

  int i,j,k,t;
  double dz,zz,ice;
  double ms,mb,dc,cs,ds;
  double vx,vy,vz;
  double bf,vxx,vyy; //Maxime
  ...
  for (i=0;i<ny;i++) {
	...
	if (vx < 0.0) {
	  dz = (double)(ice/Nz);//Maxime
	  for (k=0;k<Nz;k++) {
		zz = (double)k*dz;
		bf = (hp[i][j].ice - zz)/hp[i][j].ice ;
		if (bf < 0.0) bf = 0.0;
		else if (bf > 1.0) bf = 1.0;
		vxx = 1.2*(1-pow(bf,4))*vx + hp[i][j].vx_b; /*for equation 2*/
	    dc = dt*vxx*cells[i][j].Vs[k]/dx;/*dc<0*/
	    if (dc < -cells[i][j].Vs[k]) dc = -cells[i][j].Vs[k];
	    cells[i][j].Vs[k] += dc;
	    cells[i][j-1].Vs[k] -= dc;
	    if (k == (Nz-1)) {
	  ...
	  for (k=0;k<Nz;k++) {
		...
		vxx = 1.2*(1-pow(bf,4))*vx + hp[i][j].vx_b; /*for equation 2*/
	    dc = dt*vxx*cells[i][j-1].Vs[k]/dx;/*dc>0*/
  ...
  for (i=1;i<ny;i++) {
	...
	if (vy < 0.0) {
	  ...
	  for (k=0;k<Nz;k++) {
		zz = (double)k*dz;
		bf = (vp[i][j].ice - zz)/vp[i][j].ice ;
		if (bf < 0.0) bf = 0.0;
		else if (bf > 1.0) bf = 1.0;
		vyy = 1.2*(1-pow(bf,4))*vy + vp[i][j].vy_b; /*for equation 2*/
	    dc = dt*vyy*cells[i][j].Vs[k]/dy; /*dc<0*/
	    if (dc < -cells[i][j].Vs[k]) dc = -cells[i][j].Vs[k];
	    cells[i][j].Vs[k] += dc;
	    cells[i-1][j].Vs[k] -= dc;
	    if (k == (Nz-1)) {
	  ...
	  for (k=0;k<Nz;k++) {
		...
		vyy = 1.2*(1-pow(bf,4))*vy + vp[i][j].vy_b; /*for equation 2*/
	    dc = dt*vyy*cells[i-1][j].Vs[k]/dy; /*dc>0*/
...
void glacial_sediment_transport_old(celltype **cells,hptype **hp,vptype **vp,meshtype mesh,iproptype iprop,double dt)
{
  ...
  double dy = mesh.dy;
  int dosediment = mesh.dosediment;
  int dosubglacialdeformation = mesh.dosubglacialdeformation; //Maxime
  int Nz = 20;
  double minice = 20.0;
  double La = 3.0e3;
  double ksg = iprop.ksg*iprop.rho*mesh.gravity;
  ...
  /* do subglacial deformation */
  if (dosubglacialdeformation > 0) {
	  for (i=0;i<ny;i++) {
		for (j=1;j<nx;j++) {
			cells[i][j].sdeformation = 0.0;
		}
	  }
	  /*loop hpoints for horizontal advection*/
	  for (i=0;i<ny;i++) {
		for (j=1;j<nx;j++) {
		  if (hp[i][j].ice > minice) {
		vx = hp[i][j].vx_b; /*basal velocity*/
		if (vx < 0.0) {
		  sed = dt*vx*5.0*(1.0 - exp(-cells[i][j].sedi/5.0));
		  if (sed < -cells[i][j].sedi) sed = -cells[i][j].sedi;
		  cells[i][j].sedi += sed;
		  cells[i][j-1].sedi -= sed;
		  cells[i][j].sdeformation += sed;
		  cells[i][j-1].sdeformation -= sed;
		}
		else if (vx > 0.0) {
			sed = dt*vx*5.0*(1.0 - exp(-cells[i][j-1].sedi/5.0));
			if (sed > cells[i][j-1].sedi) sed = cells[i][j-1].sedi;
			cells[i][j-1].sedi -= sed;
			cells[i][j].sedi += sed;
			cells[i][j-1].sdeformation -= sed;
			cells[i][j].sdeformation += sed;
	  ...
	  for (i=1;i<ny;i++) {
		for (j=0;j<nx;j++) {
		  if (vp[i][j].ice > minice) {
		vy = vp[i][j].vy_b; /*basal velocity*/
		if (vy < 0.0) {
		  sed = dt*vy*5.0*(1.0 - exp(-cells[i][j].sedi/5.0));
		  if (sed < -cells[i][j].sedi) sed = -cells[i][j].sedi;
		  cells[i][j].sedi += sed;
		  cells[i-1][j].sedi -= sed;
		  cells[i][j].sdeformation += sed;
		  cells[i-1][j].sdeformation -= sed;
		}
		else if (vy > 0.0) {
			sed = dt*vy*5.0*(1.0 - exp(-cells[i-1][j].sedi/5.0));
			if (sed > cells[i-1][j].sedi) sed = cells[i-1][j].sedi;
			cells[i-1][j].sedi -= sed;
			cells[i][j].sedi += sed;
			cells[i-1][j].sdeformation -= sed;
			cells[i][j].sdeformation += sed;
