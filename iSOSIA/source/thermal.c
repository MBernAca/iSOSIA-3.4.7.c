...
void findT(long N,double H,double Ts,double Tm,double qb,double ki,double rhoi,double cpi,double Li,double hconv,double hcond,double hprod,double sliding,double sbed,double **Tcoef,double **al,long *indx,double *Tz,double *Tb,double *Ta,double Ms,double *Mb,double *sfac)
{
  ...
  for (i=1;i<(N-1);i++) {
    ...
    Tcoef[i][3] = hconv + hcond - hprod; /*load vector*/
...
void get_thermal(celltype **cells,meshtype mesh,iproptype iprop,mproptype mprop,int N,long *indx,double *Tz,double **Tcoef,double **al)
{
  ...
  int i,j;
  double ice,Ts,Tm,hcond,hconv,hprod,sliding,ts,Ms,Mb,sfac;
  double Ta,Tb;
  ...
  for (i=0;i<ny;i++) {
	...
	hcond = 0.0;
	hconv = 0.0;
	hprod =  9.82*sqrt(cells[i][j].te2)*cells[i][j].deformation/(s_per_y*Li);
	...
	findT(N,ice,Ts,Tm,qb,ki,rhoi,cpi,Li,0.0,0.0,hprod,sliding,ts,Tcoef,al,indx,Tz,&Tb,&Ta,Ms,&Mb,&sfac);
	...
	if (fabs(Tb) > 100.0) {
	  printf("  Ms = %2.2e, Mb = %2.2e, Ts = %2.2e, Tm = %2.2e, Tb = %2.2e, ice = %2.2e, sliding = %2.2e, ts = %2.2e, hcond = %2.2e, hconv = %2.2e\n",Ms,Mb,Ts,Tm,Tb,ice,sliding,ts,hcond,hconv);
	}
