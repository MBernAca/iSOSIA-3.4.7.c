...
void fixpit(int ipit,int jpit,celltype **cells) 
{
  ...
  if ((cells[ipit][jpit].fixflag <= 0) && (cells[ipit][jpit].include>0)) {
...
void get_dnetwork(celltype **cells,meshtype *mesh)
{
  ...
  for (i=0;i<ny;i++) {
    for (j=0;j<nx;j++) {
      ...
      cells[i][j].wsurf = cells[i][j].topice; //+cells[i][j].Pw;*/
    }/*j*/
  }/*i*/
  ...
  /*compute lakewater - Braun and Wilett (2013)*/
  for (i=0;i<ny;i++) {
    for (j=0;j<nx;j++) {
      cells[i][j].lakewater = cells[i][j].wsurf - cells[i][j].topice; //water lake at the ice surface or bed surface (ice-free area)
    }/*j*/
  }/*i*/
...
void pass_water(celltype **cells,meshtype *mesh,fproptype fprop,long ncascade,long **cascade,double dt)
{
  ...
  for (i=0;i<ny;i++) {
    for (j=0;j<nx;j++) {
	  ...
	  /*surface water combine with glacial hydrology some fraction of surface melt flow on top of ice*/
	  cells[i][j].water = (cells[i][j].theta*pr + cells[i][j].water2surf)*dt; //Maxime (m)
      cells[i][j].Qw = 0.0;
    }
  }
...
{
  /* Stream Power law with threshold component - Whipple 2004 */
  int i,j;
  int di,dj,ri,rj,pi,pj;
  long k,tp;
  ...
  for (i=0;i<ny;i++) {
    for (j=0;j<nx;j++) {
	  cells[i][j].water = cells[i][j].water; //Maxime - surface water combine with glacial hydrology
      cells[i][j].Qw = 0.0;
      cells[i][j].Wc = 0.0;
      cells[i][j].tau = 0.0;
      cells[i][j].Qt = 0.0;
  ...
  for (k=0;k<ncascade;k++) {
	...
	if ((tau > tau_c) && (Dg <= 0.002)) Qt = Kt*Wc/(Rb*g)*pow(tau/rho_w,1.5)*pow(1.0-0.846*pow(tau_c/tau,0.5),4.5); //Wilcock (2001)
	else if ((tau > tau_c) && (Dg > 0.002)) Qt = Kt*Wc/(Rb*g)*pow(tau/rho_w,1.5)*pow(1.0-0.846*tau_c/tau,4.5); //Wilcock (2001)
	else Qt = 0.0;
  ...
  for (k=0;k<ncascade;k++) {
	...
	/*bed slope - explicit scheme*/
	slope = (cells[di][dj].bed-cells[ri][rj].bed)/cells[di][dj].rdist;	//Maxime
...
void fluvial_transport2(celltype **cells,meshtype *mesh,fproptype fprop,long ncascade,long **cascade,double dt)
{
  ...
  for (i=0;i<ny;i++) {
    for (j=0;j<nx;j++) {
      if (cells[i][j].ice > 10.0) cells[i][j].water = cells[i][j].theta*pr*dt; //Maxime - surface water 
      else cells[i][j].water = cells[i][j].theta*pr*+cells[i][j].water*dt; //Maxime - surface water combine with glacial hydrology
      cells[i][j].Qw = 0.0;
      cells[i][j].Wc = 0.0;
      cells[i][j].tau = 0.0;
      cells[i][j].Qt = 0.0;
...
void fluvial_transport3(celltype **cells,meshtype *mesh,fproptype fprop,long ncascade,long **cascade,double dt)
{
  /*Stream power law without sediment cover function*/
  int i,j;
  int di,dj,ri,rj,pi,pj;
  long k,tp;
  double Qw,slope,Qt,sediload;
  double depo,ero,minptopo,maxdepo;
  double mean_dHs = 0.0;
  double mean_erate = 0.0;
  ...
  double Kt = fprop.Kt;
  double Ke = fprop.Ke;
  double m = fprop.m; //Maxime
  double spy = 3600.0*24.0*365.25;
  double g = 10.0;
  double Qc, maxero, fac, efac; //Maxime
  double minfac = 0.1; //Maxime
  double maxsedi; //Maxime
 ...
 /*loop cascade and compute carrying capacity*/
  for (k=0;k<ncascade;k++) {
    ...
    if (cells[di][dj].reciever >= 0) {
      ...
      /*water flux  m3/yr */
      Qw = area*cells[di][dj].water/(dt);//(spy*dt);
	  
	  /*bed slope*/
      slope = (cells[di][dj].topice+cells[di][dj].lakewater-cells[ri][rj].topice+cells[ri][rj].lakewater)/cells[di][dj].rdist;   
	  
	  /* Volumetric carrying capacity */
	  Qt = Kt*pow(Qw*dt,1.5)*slope;
	  
      /*if downhill*/
      if (slope > 0.0) {
		  
		  /*if starved*/
		  if ((Qt>cells[di][dj].streamload) && (Qt > 0.0) && (cells[di][dj].ice < 10.0)) {
			  
			  /*erode sediments */
			  if ((dosediment>0) && (cells[di][dj].sedi > 0.0)) {
				  ero = (Qt-cells[di][dj].streamload)/area; /*lift to capacity*/
				  maxero = (cells[di][dj].bed+cells[di][dj].sedi)-(cells[ri][rj].bed+cells[ri][rj].sedi);
				  if (maxero < 0.0) maxero = 0.0;
				  if (ero > maxero) ero = maxero;
				  if (ero > cells[di][dj].sedi) ero = cells[di][dj].sedi;
				  cells[di][dj].streamload += ero*area;
				  cells[di][dj].sedi -= ero; 
			  }
			  /*Erode bedrock*/
			  if (Qt > cells[di][dj].streamload) {
						efac = 1.0; //Detachment-limited
                        ero = dt*Ke*pow(slope, 1.0)*pow(Qw, m)*efac/area;
                        maxero = cells[di][dj].bed-(cells[ri][rj].bed+cells[ri][rj].sedi);
                        if (maxero < 0.0) maxero = 0.0;
                        if (ero > maxero) ero = maxero;
                        if (dosediment > 0) cells[di][dj].streamload += ero*area;
                        cells[di][dj].bed -= ero;
                        cells[di][dj].fluvial_erosion += ero;
						mean_erate += ero/dt;
                    }
		  }/*if starved*/
		  
		  /*if overloaded*/
          else if ((dosediment > 0)&&(Qt < cells[di][dj].streamload)) {
                    
			/*volume of deposited sediment*/
			depo = cells[di][dj].streamload - Qt;
			
			/*maximum sedimentation allowed*/
			maxsedi = (cells[di][dj].bed+cells[di][dj].sedi)-(cells[ri][rj].bed+cells[ri][rj].sedi);
			if (depo > maxsedi*area) depo = maxsedi*area;
			if (depo < 0.0) depo = 0.0;
			
			/*subtract from suspended sediment*/
			cells[di][dj].streamload -= depo;
			
			/*add to sediment array*/
			cells[di][dj].sedi += depo/area;
...
// {
 ...
 // /*loop cascade and compute carrying capacity*/
  ...
  // /*loop cascade again and compute carrying capacity*/
  // for (k=0;k<ncascade;k++) {
  ...
  // /*bedrock erosion*/
	// ero = Ke*pow(Qw,m)*slope;
  
    // /*River erode if ice i not present in the cell and there is no lake */
	// if ((di > 0)&&(dj > 0)&&(di < ny-1)&&(dj < nx-1)&&(cells[di][dj].ice<0.5)&&(cells[di][dj].lakewater==0)) { //Maxime
		// // mean_erate += ero;
		// // ero *= dt;
		// // if (ero > (cells[di][dj].bed-cells[ri][rj].bed)) ero = (cells[di][dj].bed-cells[ri][rj].bed);
		// // if (ero < 0.0) ero = 0.0;
		// // cells[di][dj].bed -= ero;
		// // cells[di][dj].fluvial_erate = ero;
		// // cells[di][dj].fluvial_erosion += ero;
	  // if ((ri == di+1)&&(rj == dj-1)) {
	// if (ero > (cells[di][dj].bed-cells[ri][rj].bed)/(cw*dt)) ero = (cells[di][dj].bed-cells[ri][rj].bed)/(cw*dt);
	// if (ero > (cells[di+1][dj].bed-cells[ri][rj].bed)/(sw*dt)) ero = (cells[di+1][dj].bed-cells[ri][rj].bed)/(sw*dt);
...
void fluvial_transport4(celltype **cells,meshtype *mesh,fproptype fprop,long ncascade,long **cascade,double dt)
{
  /*Stream power law without sediment cover function*/
  int i,j;
  int di,dj,ri,rj,pi,pj;
  ...
  double minfac = 0.1; //Maxime
  double maxsedi; //Maxime
  double threshold_sp = fprop.t_sp * area * fprop.pr; // Threshold for stream erosion, is n times the minimum water discharge
  double nci = (*mesh).nci;
 ...
 /*loop cascade and compute carrying capacity*/
  for (k=0;k<ncascade;k++) {
    ...
    if (cells[di][dj].reciever >= 0) {
	  ...
	  /*bed slope*/
      //slope = (cells[di][dj].topice+cells[di][dj].lakewater-cells[ri][rj].topice+cells[ri][rj].lakewater)/cells[di][dj].rdist;   
	  slope = (cells[di][dj].topice-cells[ri][rj].topice)/cells[di][dj].rdist;  
	  /* Volumetric carrying capacity */
	  Qt = Kt*pow(Qw*dt,1.5)*slope;
	  
      /*if downhill*/
      if ((slope > 0.0) && (cells[di][dj].include > 0.0))  {
		  ...
		  if ((Qt>cells[di][dj].streamload) && (Qt > 0.0) && (cells[di][dj].ice < 10.0)) {
			  ...
			  /*Erode bedrock*/
			  if ((Qt > cells[di][dj].streamload) && (Qw*dt > threshold_sp))  {
                        fac = cells[di][dj].streamload/Qt;
                        if (fac < minfac) fac = minfac;
                        efac = 4.0*fac*(1.0 - fac);
						//efac = 1.0; //Detachment-limited
                        ero = dt*Ke*pow(slope, 1.0)*pow(Qw, m)*efac/area;
                        maxero = cells[di][dj].bed-(cells[ri][rj].bed+cells[ri][rj].sedi);
                        if (maxero < 0.0) maxero = 0.0;
  ...
  (*mesh).mean_dHs_fluvial = mean_dHs/((double)(*mesh).nc*dt);
  // (*mesh).mean_fluvial_erate = mean_erate/((double)(*mesh).nc);
  (*mesh).mean_fluvial_erate = mean_erate/(double)(nci);
