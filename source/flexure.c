...
void flexural_isostasy(celltype **cells,meshtype *mesh,iproptype iprop,flexproptype flexprop,double **W, double dt_isos)
{
  ...
  double isostasy_rate = 0.0; //Maxime
  double mean_isos_prev = (*mesh).mean_isostasy; //Maxime
  ...
  for (i=0;i<ny;i++) {
    for (j=0;j<nx;j++) {
      loadg[i+1][j+1] = grav*(cells[i][j].ice*rho_i+cells[i][j].sedi*rho_s-(cells[i][j].abrasion+cells[i][j].fluvial_erosion+cells[i][j].hillslope_erosion)*rho_r); 
  ...
  (*mesh).isostasy_rate = (mean_isostasy-mean_isos_prev) / (dt_isos+tiny); //Maxime
...
// #pragma omp for schedule(static)
  // for (j=0;j<nx;j++) {
    // qm = 0.0;
    // for (i=0;i<ny;i++) {
      // qm += -rho_i*(cells[i][j].ice-cells[i][j].iniice)+rho_r*(cells[i][j].periglacial_erosion+cells[i][j].abrasion+cells[i][j].quarrying+cells[i][j].hillslope_erosion+cells[i][j].fluvial_erosion);
    // }
