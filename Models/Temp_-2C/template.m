...
for i=1:length(time) 
    if time(i) < 1.7e6
        Temp(i) = 6.5+ -1.*sawtooth(shift+2.*pi.*time(i)./41e3,0.5)-(tempdrop/SPM.mesh.maxtime)*time(i);  %sea level temp 
    elseif time(i) >= 1.7e6 & time(i) < 2e6
        Temp(i) = 6.5+ -1.*(1+((time(i)-1.5e6)/0.5e6)*1).*sawtooth(shift+2.*pi.*time(i)./100e3,0.8)-(tempdrop/SPM.mesh.maxtime)*time(i);
    elseif time(i) >=2e6
        Temp(i) = 6.5 + -1.*2*sawtooth(shift+2*pi*time(i)/100e3,0.8)-(2.5/SPM.mesh.maxtime)*time(i);
    end
end   
figure
plot(time,Temp)
